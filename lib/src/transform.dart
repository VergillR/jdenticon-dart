...
class Transform {
  ...
  Point transformPoint(double x, double y, [double w, double h]) {
    ...
    return  this._rotation == 1 ? Point(right - y - height, this._y + x) :
            this._rotation == 2 ? Point(right - x - width, bottom - y - height) :
            this._rotation == 3 ? Point(this._x + y, bottom - x - width) :
